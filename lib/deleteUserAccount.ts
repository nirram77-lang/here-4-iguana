...
export const deleteUserAccount = async (userId: string): Promise<{
  ...
  try {
    ...
    // Step 5: ✅ CRITICAL - Keep phoneIdentity with timer!
    // This prevents the user from bypassing the 2-hour lock by deleting and re-registering
    if (hasActiveTimer) {
      console.log(`🔒 Preserving phoneIdentity with ${Math.floor(timerRemaining / 60)} minutes remaining`)
      console.log('   This prevents re-registration exploit')
    } else {
      console.log('✅ No active timer - user can re-register freely')
    }
